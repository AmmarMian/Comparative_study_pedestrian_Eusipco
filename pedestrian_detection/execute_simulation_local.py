...
if __name__ == '__main__':
    ...
    if os.path.isfile(args.simulation_setup):
        ...
        with open(execution_script_path, 'w') as f:
            ...
            # ---------------------------------------------------------------------------------------------------------------------------
            # Executing K-fold cross-validation training on training dataset
            # ---------------------------------------------------------------------------------------------------------------------------
            temp = os.path.join(folder_of_present_script, "Scripts/compute_training_local.py").replace(" ", "\\ ")

            k_fold_training_script = f"python3 {temp} " \
                               f" {path_to_data_storage_file} {args.simulation_setup} "

            if simulation_setup['global_setups']['Triton_jobs']['parallel']:
                k_fold_training_script += f" -p -j {simulation_setup['global_setups']['Triton_jobs']['number_of_jobs']}"
