# -*- coding: utf-8 -*-
# @Author: miana1
# @Description: Gridsearch cv for Riemannian RBF kernel to generate a plot
# @Date:   2020-01-31 13:10:02
# @E-mail: ammar.mian@aalto.fi
# @Last Modified by:   miana1
# @Last Modified time: 2020-02-03 13:31:01
# ----------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    ax.plot(-gs.cv_results_['mean_test_False positives per window'], -gs.cv_results_['mean_test_Miss rate'], marker='x',
        linestyle='None')
    ax.set_xscale('log')
    #plt.ylim([np.min(-gs.cv_results_['mean_test_Miss rate']), np.max(-gs.cv_results_['mean_test_Miss rate'])])
    plt.savefig('results_riemannian_rbf_svc_%s.png' % dataset['name'])